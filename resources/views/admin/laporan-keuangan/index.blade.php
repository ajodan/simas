            ...
            <div class="card-body hover-scroll-overlay-y">
                <form class="form-data" enctype="multipart/form-data">
                    ...
                    <div class="mb-10">
                        ...
                        <select id="jenis_laporan_id" class="form-control" name="jenis_laporan_id">
                            <option value="">Pilih Jenis Laporan</option>
                            @foreach($jenisLaporans as $jenis)
                                <option value="{{ $jenis->id }}">{{ $jenis->jenis_laporan }}</option>
                            @endforeach
                        </select>
                        <small class="text-danger jenis_laporan_id_error"></small>
                    </div>
                    <div class="mb-10">
                        <label class="form-label">Judul Laporan</label>
                        <input type="text" id="jenis_laporan" class="form-control" name="jenis_laporan">
                        <small class="text-danger jenis_laporan_error"></small>
                    </div>

                    <div class="mb-10">
                        <label class="form-label">Saldo Akhir</label>
                        <input type="number" step="0.01" id="saldo_akhir" class="form-control" name="saldo_akhir">
                        <small class="text-danger saldo_akhir_error"></small>
                    </div>

                    <div class="mb-10">
                        <label class="form-label">Upload File</label>
                        <input type="file" accept=".pdf,.doc,.docx,.xls,.xlsx" id="upload_file" class="form-control" name="upload_file">
                        <small class="text-danger upload_file_error"></small>
...
@section('script')
    <script>
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            if (type == 'add') {
                control.submitFormMultipartData('/admin/laporan-keuangan-add', 'Tambah',
                    'Laporan Keuangan',
                    'POST');
            } else {
                let uuid = $("input[name='uuid']").val();
                control.submitFormMultipartData('/admin/laporan-keuangan-update/' + uuid,
                    'Update',
                    'Laporan Keuangan', 'POST');
            }
        });

        $(document).on('click', '#button-side-form', function(e) {
            $(".form-data").attr('data-type', 'add');
            $(".title_side_form").html(`Tambah Laporan Keuangan`);
            $(".text-danger").html("");
            // Reset form
            $(".form-data")[0].reset();
        ...
        $(document).on('click', '.button-update', function(e) {
            ...
            $(".title_side_form").html(`Update Laporan Keuangan`);
            $(".text-danger").html("");
            let url = '/admin/laporan-keuangan-show/' + $(this).attr('data-uuid');
            $.ajax({
                url: url,
                method: "GET",
                    ...
                    if (res.success == true) {
                        $.each(res.data, function(x, y) {
                            ...
                            if ($("input[name='" + x + "']").is(":radio")) {
                                ...
                                fileInfoContainer.html(`<p>File saat ini: <a href="/public/laporan-keuangan/${y}" target="_blank">${y}</a></p>`);
                            } else {
                                $("input[name='" + x + "']").val(y);
                                $("select[name='" + x + "']").val(y);
        ...
        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/admin/laporan-keuangan-delete/' + $(this).attr('data-uuid');
        ...
        const initDatatable = async () => {
            ...
            $('#kt_table_data').DataTable({
                ...
                processing: true,
                ajax: '/admin/laporan-keuangan-get',
                columns: [{
                    data: null,
                    ...
                    render: function(data, type, row, meta) {
                        if (data) {
                            return `<a href="/public/laporan-keuangan/${data}" target="_blank" class="btn btn-sm btn-light"><i class="fas fa-download"></i> Download</a>`;
                        }
                        return '-';
                    }
                }, {
                ...
                columnDefs: [{
                    ...
                    render: function(data, type, full, meta) {
                        return `
                            ...
                            <a href="javascript:;" type="button" data-uuid="${data}" data-label="Laporan Keuangan" class="btn btn-danger button-delete btn-icon btn-sm">
                                <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
