            ...
            <div class="row mt-5">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Kalender Agenda</h3>
                    </div>
                    <div class="card-body">
                        <div class="container mx-auto p-6">
                            <div class="bg-white shadow rounded p-3">
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    <button id="prev-month" data-month="{{ $currentMonth->copy()->subMonth()->month }}" data-year="{{ $currentMonth->copy()->subMonth()->year }}" class="btn btn-light btn-sm">&laquo; Sebelumnya</button>
                                    <h2 id="current-month-title" class="h5 mb-0">{{ $currentMonth->format('F Y') }}</h2>
                                    <button id="next-month" data-month="{{ $currentMonth->copy()->addMonth()->month }}" data-year="{{ $currentMonth->copy()->addMonth()->year }}" class="btn btn-light btn-sm">Berikutnya &raquo;</button>
                                </div>
...
<script>
    ...
    $(document).on('click', '#button-side-form', function() {
        control.overlay_form('Tambah', 'Agenda');
    })
    ...
    $(document).on('submit', ".form-data", function(e) {
        ...
        if (type == 'add') {
            control.submitFormMultipartData('/admin/kegiatan/agenda-add', 'Tambah',
                'Agenda',
                'POST');
        } else {
            let uuid = $("input[name='uuid']").val();
            control.submitFormMultipartData('/admin/kegiatan/agenda-update/' + uuid,
                'Update',
                'Agenda', 'POST');
        }
    });
    ...
    $(document).on('click', '.button-update', function(e) {
        e.preventDefault();
        $(".title_side_form").html(`Update Agenda`);
        $(".text-danger").html("");
        let url = '/admin/kegiatan/agenda-show/' + $(this).attr('data-uuid');
        $.ajax({
            url: url,
            method: "GET",
                    ...
                    $.each(res.data, function(x, y) {
                        ...
                        if ($("input[name='" + x + "']").is(":radio")) {
                            ...
                            logoInfoContainer.html(
                                `<img id="img-foto" src="/public/agenda/${y}" style="max-width:100%;">`
                            );
    ...
    $(document).on('click', '.button-delete', function(e) {
        e.preventDefault();
        let url = '/admin/kegiatan/agenda-delete/' + $(this).attr('data-uuid');
    ...
    const initDatatable = async () => {
        ...
        $('#kt_table_data').DataTable({
            ...
            processing: true,
            ajax: '/admin/kegiatan/agenda-get',
            columns: [{
                data: null,
            ...
            columnDefs: [{
                ...
                render: function(data, type, full, meta) {
                    return `
                        ...
                        <a href="javascript:;" type="button" data-uuid="${data}" data-label="Agenda" class="btn btn-danger button-delete btn-icon btn-sm">
                            <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    $(document).ready(function() {
        ...
        // Handle click on date cell to show agenda details
        $(document).on('click', '#calendar-container td[data-date]', function() {
            var date = $(this).data('date');
            $('#agendaDetailModalLabel').text('Detail Agenda - ' + date);
            $('#agenda-detail-list').empty();

            $.ajax({
                url: '/admin/kegiatan/agenda-by-date',
                type: 'GET',
                data: { date: date },
                success: function(response) {
                    if (response.success) {
                        ...
                        if (agendas.length > 0) {
                            agendas.forEach(function(agenda) {
                                ...
                                listItem.append('<h6>' + agenda.judul + '</h6>');
                                listItem.append('<p>' + agenda.deskripsi + '</p>');
                                $('#agenda-detail-list').append(listItem);
                            });
                        } else {
                            $('#agenda-detail-list').append('<li class="list-group-item">Tidak ada agenda pada tanggal ini.</li>');
                        }
                        var modal = new bootstrap.Modal(document.getElementById('agendaDetailModal'));
                        modal.show();
                    } else {
