            ...
            <div class="card-body hover-scroll-overlay-y">
                <form class="form-data">
                    ...
                    <div class="mb-10">
                        ...
                        <input type="text" id="jenis_inventaris" class="form-control" name="jenis_inventaris">
                        <small class="text-danger jenis_inventaris_error"></small>
                    </div>
...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            control.overlay_form('Tambah', 'Jenis Inventaris');
        ...
        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            $(".title_side_form").html(`Update Jenis Inventaris`);
            $(".text-danger").html("");
            $(".form-data").attr('data-type', 'update');
            // Clear form fields except uuid
            $(".form-data")[0].reset();
            let uuid = $(this).attr('data-uuid');
            let url = '/admin/jenisinventaris-show/' + uuid;
            $.ajax({
                url: url,
                method: "GET",
                        ...
                        $.each(res.data, function(x, y) {
                            ...
                            if ($("input[name='" + x + "']").is(":radio")) {
                                ...
                                logoInfoContainer.html(
                                    `<img id="img-foto" src="/public/jenisinventaris/${y}" style="max-width:100%;">`
                                );
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            if (type == 'add') {
                control.submitFormMultipartData('/admin/jenisinventaris-add', 'Tambah',
                    'Jenis Inventaris',
                    'POST');
            } else {
                let uuid = $("input[name='uuid']").val();
                control.submitFormMultipartData('/admin/jenisinventaris-update/' + uuid,
                    'Update',
                    'Jenis Inventaris', 'POST');
            }
        });
        ...
        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/admin/jenisinventaris-delete/' + $(this).attr('data-uuid');
        ...
        const initDatatable = async () => {
            ...
            $('#kt_table_data').DataTable({
                ...
                processing: true,
                ajax: '/admin/jenisinventaris-get',
                columns: [{
                    data: null,
