            ...
            <div class="card-body hover-scroll-overlay-y">
                <form class="form-data" enctype="multipart/form-data">
                    ...
                    <div class="mb-10">
                        ...
                        <input type="text" id="target_dana" class="form-control" name="target_dana">
                        <small class="text-danger target_dana_error"></small>
                    </div>

                    <div class="mb-10">
                        <label class="form-label">Tanggal Mulai</label>
                        <input type="text" id="tanggal_mulai" class="form-control kt_datepicker_7" name="tanggal_mulai">
                    ...
                    <div class="mb-10">
                        ...
                        <input type="file" accept="image/*" id="gambar" class="form-control" name="gambar">
...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            control.overlay_form('Tambah', 'Campaign Donasi');
        })

        $('#target_dana').on('input', function() {
            let value = $(this).val();
            if (value !== "") {
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            if (type == 'add') {
                control.submitFormMultipartData('/admin/data-donasi/campaign-donasi-add', 'Tambah',
                    'Campaign Donasi',
                    'POST');
            } else {
                let uuid = $("input[name='uuid']").val();
                control.submitFormMultipartData('/admin/data-donasi/campaign-donasi-update/' + uuid,
                    'Update',
                    'Campaign Donasi', 'POST');
            }
        });

        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            let url = '/admin/data-donasi/campaign-donasi-show/' + $(this).attr('data-uuid');
            control.overlay_form('Update', 'Campaign Donasi', url);
        })

        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/admin/data-donasi/campaign-donasi-delete/' + $(this).attr('data-uuid');
            let label = $(this).attr('data-label');
            control.ajaxDelete(url, label)
        })

        $(document).on('click', '.button-donasi', function(e) {
            e.preventDefault();
            let label = $(this).attr('data-label');

            $.ajax({
                type: 'GET',
                url: '/admin/data-donasi/update-tombol-campaign/' + $(this).attr('data-uuid'),
                contentType: false,
                processData: false,
        ...
        $(document).on('click', '.button-list-pendonasi', function(e) {
            e.preventDefault();
            window.location.href = '/admin/data-donasi/list-donasi-campaign/' + $(this).attr('data-uuid');
        })
        ...
        const initDatatable = async () => {
            ...
            $('#kt_table_data').DataTable({
                ...
                processing: true,
                ajax: '/admin/data-donasi/campaign-donasi-get',
                columns: [{
                    data: null,
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        result =
                            `
                                <!--begin::Overlay-->
                                <a class="d-block overlay fancybox" data-fancybox="lightbox-group" href="{{ asset('/storage/public/campaign/${data}') }}">
                                    <!--begin::Image-->
                                    <div class="overlay-wrapper bgi-no-repeat bgi-position-center bgi-size-cover card-rounded min-h-100px"
                                        style="background-image:url('/storage/public/campaign/${data}')">
                                    </div>
                                    <!--end::Image-->
                ...
                columnDefs: [{
                    ...
                    render: function(data, type, full, meta) {
                        return `
                            ...
                            <a href="javascript:;" type="button" data-uuid="${data}" data-label="Campaign Donasi" class="btn btn-danger button-delete btn-icon btn-sm">
                                <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <a href="javascript:;" type="button" data-uuid="${data}" data-label="${full.status == false ? 'Buka' : 'Tutup'}" class="btn btn-success button-donasi btn-sm">
                                    ${full.status == false ? 'Buka Donasi' : 'Tutup Donasi'}
                                </a>

                            <a href="javascript:;" data-uuid="${data}" class="btn btn-info btn-sm button-list-pendonasi">
                                    List Pendonasi </a>
                        `;
                    },
                }],
