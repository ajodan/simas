            ...
            <div class="card-body hover-scroll-overlay-y">
                <form class="form-data" enctype="multipart/form-data">
                    ...
                    {{-- <div class="mb-10">
                        ...
                        <select name="data_jamaah_id" class="form-select" data-control="select2" id="data_jamaah_select"
                            data-placeholder="Pilih Data Jamaah">
                        </select>
                        <small class="text-danger data_jamaah_id_error"></small>
                    </div>--}}
...
@section('script')
    <script>
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            if (type == 'add') {
                control.submitFormMultipartData('/admin/users-add', 'Tambah',
                    'User',
                    'POST');
            } else {
                let uuid = $("input[name='uuid']").val();
                control.submitFormMultipartData('/admin/users-update/' + uuid,
                    'Update',
                    'User', 'POST');
            }
        });
        ...
        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            let url = '/admin/users-show/' + $(this).attr('data-uuid');
            control.overlay_form('Update', 'User', url);
        })

        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/admin/users-delete/' + $(this).attr('data-uuid');
        ...
        $(function() {
            control.push_select3(roles, '#role_select');
            loadJamaah();
            initDatatable();
        });
        ...
        const loadJamaah = () => {
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        let options = [{ id: '', text: 'Pilih Data Jamaah' }];
                        response.data.forEach(item => {
                            options.push({ id: item.id, text: item.nama });
                        });
                        control.push_select3(options, '#data_jamaah_select');
                    }
                }
            });
        };

        $(document).on('change', '#data_jamaah_select', function() {
