            ...
            <div class="card-body hover-scroll-overlay-y">
                <form class="form-data" enctype="multipart/form-data">
                    ...
                    <div class="mb-10">
                        ...
                        <select class="form-control" name="dokumentasi_id">
                            <option value="">Pilih Dokumentasi</option>
                            @foreach(\App\Models\Dokumentasi::all() as $dok)
                            <option value="{{ $dok->id }}">{{ $dok->judul }}</option>
                            @endforeach
                        </select>
                        <small class="text-danger dokumentasi_id_error"></small>
                    </div>
                    @else
                    <input type="hidden" name="dokumentasi_id" value="{{ $dokumentasi->id }}">
                    <div class="mb-10">
                        <label class="form-label">Dokumentasi</label>
                        <input type="text" class="form-control" value="{{ $dokumentasi->judul }}" readonly>
                    </div>
                    @endif
                    ...
                    <div class="mb-10">
                        ...
                        <div class="mt-3" id="fotoPreview"></div>
                    </div>
...
@section('script')
    <script>
        ...
        function previewImage(input) {
            ...
            if (input.files && input.files[0]) {
                ...
                if (file.type.startsWith('image/')) {
                    ...
                    error.text('File harus berupa gambar.');
                    preview.html('');
                }
            } else {
                preview.html('');
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            if (type == 'add') {
                control.submitFormMultipartData('/admin/sub-dokumentasi-add', 'Tambah',
                    'Sub Dokumentasi',
                    'POST');
            } else {
                let id = $("input[name='id']").val();
                control.submitFormMultipartData('/admin/sub-dokumentasi-update/' + id,
                    'Update',
                    'Sub Dokumentasi', 'POST');
            }
        });
        ...
        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/admin/sub-dokumentasi-delete/' + $(this).attr('data-id');
        ...
        const initDatatable = async () => {
            ...
            $('#kt_table_data').DataTable({
                ...
                processing: true,
                ajax: '/admin/sub-dokumentasi-get{{ $dokumentasi ? '?dokumentasi_uuid=' . $dokumentasi->uuid : '' }}',
                columns: [{
                    data: null,
                ...
                columnDefs: [{
                    ...
                    render: function(data, type, full, meta) {
                        return `
                            <a href="javascript:;" type="button" data-id="${data}" data-kt-drawer-show="true" data-kt-drawer-target="#side_form" class="btn btn-warning button-update btn-icon btn-sm">
                            ...
                            <a href="javascript:;" type="button" data-id="${data}" data-label="Sub Dokumentasi" class="btn btn-danger button-delete btn-icon btn-sm">
                                <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
